...
py_repr!(PythonAlgebra, "FreedPythonAlgebra", {
    Ok(format!("PythonAlgebra(p={})", *inner.prime))
});

crate::algebra_bindings!(
    PythonAlgebra,
    PythonAlgebraRust,
    PythonElement,
    "PythonElement"
...
impl Algebra for PythonAlgebraRust {
    ...
    fn compute_basis(&self, degree: i32) {
        if self.compute_basis.is_none() {
            ...
            if let Err(e) = result {
                eprintln!("Error occurred in call compute_basis({}):", degree);
                e.print(py);
                panic!();
            }
        }
    ...
    fn dimension(&self, degree: i32, excess: i32) -> usize {
        ...
        let result = self
            .get_dimension
            .call1(py, (degree, excess))
            .unwrap()
            .extract(py);
        match result {
            Err(e) => {
                eprintln!("Error occurred in call dimension({}, {}):", degree, excess);
                e.print(py);
                panic!();
            }
            Ok(value) => value,
    ...
    fn multiply_basis_elements(
        ...
        let result = self.multiply_basis_elements.call1(
            py,
            (
                FpVector::wrap(result, Arc::downgrade(&temp_arc)),
                coeff,
                r_degree,
                r_idx,
                s_degree,
        ...
        if let Err(e) = result {
            eprintln!(
                "Error occurred in call multiply_basis_elements(result, {}, {}, {}, {}, {}, {}):",
    ...
    fn basis_element_to_string(&self, degree: i32, idx: usize) -> String {
        ...
        match result {
            Err(e) => {
                eprintln!(
                    "Error occurred in call basis_element_to_string({}, {}):",
                    degree, idx
                );
                e.print(py);
                panic!();
